...
.then(() => {
  ...
  t('ParityManager{}', async t => {
    ...
    t.test('addWordToIndex() returns an error code for all expected errors.', async t => {
      parityMngr.words = [['test']];
      t.is(parityMngr.addWordToIndex('blah', -1), ParityErrorCode.IndexLessThanZero);
      t.is(parityMngr.addWordToIndex('test',  0), ParityErrorCode.AlreadyExists);
      t.is(parityMngr.addWordToIndex('blah', 10), ParityErrorCode.IndexNotFound);
      parityMngr.words = [];
    });
